...
describe("GoalManagementTool", () => {
  ...
  itMutable("fetches goals from the test budget", async () => {
    const tool = new GoalManagementTool();
    ...
    if ((result as any[]).length > 0) {
      expect((result as any[])[0]).toHaveProperty("category_id");
      expect((result as any[])[0]).toHaveProperty("goal_type");
      expect((result as any[])[0]).toHaveProperty("target");
    }
  });
}); 