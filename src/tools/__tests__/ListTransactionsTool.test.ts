...
import ListTransactionsTool from "../ListTransactionsTool";
...
describe("ListTransactionsTool", () => {
  it("validates schema for required budgetId", () => {
    const tool = new ListTransactionsTool();
  ...
  itMutable("fetches transactions from the test budget", async () => {
    const tool = new ListTransactionsTool();
    ...
    if ((result as any[]).length > 0) {
      ...
      expect((result as any[])[0]).toHaveProperty("date");
