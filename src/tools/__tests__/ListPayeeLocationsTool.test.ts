...
import ListPayeeLocationsTool from "../ListPayeeLocationsTool";
...
describe("ListPayeeLocationsTool", () => {
  it("validates schema for required budgetId", () => {
    const tool = new ListPayeeLocationsTool();
  ...
  itMutable("fetches payee locations from the test budget", async () => {
    const tool = new ListPayeeLocationsTool();
    ...
    if ((result as any[]).length > 0) {
      ...
      expect((result as any[])[0]).toHaveProperty("payee_id");
      expect((result as any[])[0]).toHaveProperty("latitude");
      expect((result as any[])[0]).toHaveProperty("longitude");
    }
  });
}); 