...
import ListPayeesTool from "../ListPayeesTool";
...
describe("ListPayeesTool", () => {
  ...
  itMutable("fetches payees from the test budget", async () => {
    const tool = new ListPayeesTool();
    ...
    if ((result as any[]).length > 0) {
      ...
      expect((result as any[])[0]).toHaveProperty("transfer_account_id");
    }
  });
}); 